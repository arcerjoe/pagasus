(function($){
	...
	this.each(function(){
		...
		if(undefined==$(self).attr("lazysrc")){
			$(self).attr("lazysrc", $(self).attr("src"));
		...
		$(self).one("appear", function(){
			if(!this.loaded || 1 == 1){
				$("<img />").bind("load", function(){
					$(self).hide().attr("src", $(self).attr("lazysrc"))[settings.effect](settings.effectspeed);
					self.loaded=true;
...
//不使用延时加载的图片
$("img:not(.lazynot)").lazyload({
	effect : "fadeIn",
	failurelimit:150
}).attr('src','/static/asset/dest/img/wt.png');
